import { useMemo, useState } from 'react'; import { Pressable, StyleSheet, View } from 'react-native'; import { router, useLocalSearchParams } from 'expo-router'; import * as Location from 'expo-location'; import * as Haptics from 'expo-haptics'; import { collection, limit, orderBy, query } from 'firebase/firestore'; import { Ionicons } from '@expo/vector-icons';
...
return <Screen header={<OrderHeader step={1} title="Delivery location" />} contentStyle={styles.content}><Text variant="h1">Where should we deliver?</Text><Text color="#64748B">Choose a precise, accessible point for the delivery vehicle.</Text><Input icon="search-outline" placeholder="Search address" value={search} onChangeText={find} />{suggestions.length > 0 && <Card style={styles.suggestions}>{suggestions.map((item) => <Pressable key={item.placeId} onPress={() => chooseSuggestion(item)} style={styles.suggestion}><Ionicons name="location-outline" size={19} color={colors.primary} /><Text style={styles.flex}>{item.description}</Text></Pressable>)}</Card>}<Button title="Use my current location" variant="outline" icon="locate-outline" loading={busy} onPress={useCurrentLocation} />{error ? <Text color={colors.danger}>{error}</Text> : null}{saved.loading ? <LoadingState label="Loading saved locations…" /> : saved.data.length > 0 && <><Text variant="h3">Saved locations</Text><View style={styles.saved}>{saved.data.map((address) => <Pressable key={address.id} onPress={() => setSelected(address)}><Card style={[styles.address, selected?.id === address.id && styles.selected]}><Ionicons name={address.label.toLowerCase().includes('home') ? 'home-outline' : 'business-outline'} size={21} color={colors.primary} /><View style={styles.flex}><Text variant="label">{address.label}</Text><Text variant="caption" color="#64748B">{address.formattedAddress}</Text></View></Card></Pressable>)}</View></>}{selected && <Card style={styles.mapCard}><LocationMap location={selected.location} onChange={(location) => setSelected({ ...selected, location })} /><View style={styles.mapCopy}><Text variant="label">{selected.label}</Text><Text variant="caption" color="#64748B">{selected.formattedAddress}</Text></View></Card>}<Button title="Continue to quantity" disabled={!selected} onPress={next} /></Screen>; }
const styles = StyleSheet.create({ content: { maxWidth: 900, gap: spacing.lg }, flex: { flex: 1 }, suggestions: { gap: 0, paddingVertical: 4 }, suggestion: { flexDirection: 'row', gap: spacing.md, padding: spacing.md, alignItems: 'center', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#E2E8F0' }, saved: { gap: spacing.sm }, address: { flexDirection: 'row', gap: spacing.md, alignItems: 'center' }, selected: { borderColor: colors.primary, backgroundColor: '#FFF9F2' }, mapCard: { padding: 0, overflow: 'hidden' }, mapCopy: { padding: spacing.lg, gap: 4 } });
