...
export function Screen({ children, scroll = true, contentStyle, refreshControl, header, edges = ['top'] }: PropsWithChildren<{ scroll?: boolean; contentStyle?: StyleProp<ViewStyle>; refreshControl?: ScrollViewProps['refreshControl']; header?: ReactNode; edges?: Edge[] }>) {
  ...
  return <SafeAreaView edges={edges} style={[styles.root, { backgroundColor: theme.colors.background }]}>{header}{body}</SafeAreaView>;
}
